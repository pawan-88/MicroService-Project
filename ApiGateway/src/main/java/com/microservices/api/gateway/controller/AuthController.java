...
import com.microservices.api.gateway.model.AuthResponse;
...
public class AuthController {
    ...
    public ResponseEntity<AuthResponse> login(
        ...
        List<String> authroties = user.getAuthorities().stream().map(grantedAuthority -> {
            return grantedAuthority.getAuthority();
        }).collect(Collectors.toList());

        authResponse.setAuthorities(authroties);
